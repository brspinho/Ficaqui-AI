...
def create_pdf(df, ai_diagnosis):
    ...
    # --- HEADER / CABEÇALHO (Estilo GridScope Boxed) ---
    w1, w2, w3 = 60, 80, 50 # Total 190mm
    h_box = 30
    ...
    # Desenha os boxes (Bordas Pretas)
    ...
    if os.path.exists(logo_path):
        # Centraliza o logo no box 1
        pdf.image(logo_path, x=15, y=12, h=45) 
        
    # Box 2: Titulo Centralizado
    ...
    # Box 3: Metadados (No Relatorio e Data)
    ...
    # Faixa de Parametros (Sub-header)
    ...
    pdf.set_xy(10, 55) # Reinicia o cursor abaixo do cabeçalho completo
    ...
    # --- TABELA DE AMOSTRA (SINTÉTICA) ---
    pdf.set_font("helvetica", 'B', 8)
    pdf.set_fill_color(240, 240, 240)
    
    # Headers com larguras calculadas para caber em A4 (~190mm total)
    ...
    # Mostra top 25 imóveis no PDF para dar volume ao relatório executivo
    for i, row in df.head(25).iterrows():
        rua_val = strip_accents(str(row['rua'])[:30]) # Corta p/ n transbordar
    ...
    # --- PARECER DE INTELIGÊNCIA ARTIFICIAL (O GRAN FINALE) ---
    pdf.set_font("helvetica", 'B', 14)
    pdf.set_text_color(0, 77, 64)
    pdf.cell(0, 10, txt="Parecer Estatégico (AI Urbanist Analysis)", ln=True)
